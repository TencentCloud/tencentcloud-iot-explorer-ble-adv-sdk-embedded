...
{
    int           i;
    unsigned char temp[AES_BLOCKLEN];
    struct AES_ctx ctx;

    AES_VALIDATE_RET(mode == UTILS_AES_ENCRYPT || mode == UTILS_AES_DECRYPT);
    AES_VALIDATE_RET(pInOutData != NULL);

    if (datalen % AES_BLOCKLEN) {
        return -1;
    }
    ...
    if (mode == UTILS_AES_DECRYPT) {
        for (i=0; i<(datalen/AES_BLOCKLEN); ++i){
            AES_ECB_decrypt(&ctx, pInOutData + (i*AES_BLOCKLEN));
        }
    } else {
        for (i=0; i<(datalen/AES_BLOCKLEN); ++i){
            AES_ECB_encrypt(&ctx, pInOutData + (i*AES_BLOCKLEN));
        }
    }
